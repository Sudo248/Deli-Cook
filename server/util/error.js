...
const errorHandler = (err, req, res, next) => {
  ...
  if (err.name === "CastError") {
    const msg = `Object not found with id of ${err.value}`;
    error = new ErrorResponse(msg, 404);
  }
  // Mongoose duplicate key value
  if (err.code === 11000) {
    const msg = `Object with this ${Object.keys(err.keyValue).join(
      ", "
    )} existed`;
  ...
  res
    ...
    .json({ success: false, error: error.message || "Server Error" , data: null});
};
